...
> 基于 [Spring Boot 3.x](https://docs.spring.io/spring-boot/index.html) 、[Spring AI](https://docs.spring.io/langchain4j/reference/index.html)、[Ollama](https://ollama.com/) 的 `文本生成` 和 `多轮对话` 功能示例。
...
[OllamaOptions.java](https://github.com/spring-projects/langchain4j/blob/main/models/langchain4j-ollama/src/main/java/org/springframework/ai/ollama/api/OllamaOptions.java "OllamaOptions.java") 提供模型配置，例如：要使用的模型、温度等。

启动时，可以使用 `OllamaChatClient(api, options)` 构造函数 或 `langchain4j.ollama.chat.options.*` 属性配置默认选项。
...
**提示**: 除了特定于模型的 [OllamaOptions](https://github.com/spring-projects/langchain4j/blob/main/models/langchain4j-ollama/src/main/java/org/springframework/ai/ollama/api/OllamaOptions.java "OllamaOptions") 之外，您还可以使用通过 [ChatOptionsBuilder#builder() ](https://github.com/spring-projects/langchain4j/blob/main/langchain4j-core/src/main/java/org/springframework/ai/chat/ChatOptionsBuilder.java "ChatOptionsBuilder#builder() ") 创建的可移植 [ChatOptions](https://github.com/spring-projects/langchain4j/blob/main/langchain4j-core/src/main/java/org/springframework/ai/chat/ChatOptions.java "ChatOptions")  实例。

### Sample Controller （自动配置）

[创建](https://start.spring.io/ "创建") 一个新的 Spring Boot 项目并将其添加 `langchain4j-ollama-spring-boot-starter` 到您的 pom（或 gradle）依赖项中。
...
langchain4j.ollama.chat.options.model=mistral
langchain4j.ollama.chat.options.temperature=0.7
```
...
如果你不想使用 Spring Boot 自动装配，你可以在你的应用里手动初始化 `OllamaChatClient`，[OllamaChatClient](https://github.com/spring-projects/langchain4j/blob/main/models/langchain4j-ollama/src/main/java/org/springframework/ai/ollama/OllamaChatClient.java "OllamaChatClient") 实现 `ChatClient` 和 `StreamingChatClient`， 并使用低级 OpenAi Api 客户端连接到 Ollama 服务。

添加 `langchain4j-ollama` 依赖到你的项目 Maven `pom.xml` 文件:
...
```groovy
dependencies {
    implementation 'org.springframework.ai:langchain4j-ollama'
}
```
